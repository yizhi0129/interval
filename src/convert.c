...
INF_SUP CR_IS(C_R int_cr)
{
    ...
    double sup = int_cr.center + int_cr.radius;
    fesetround(FE_DOWNWARD);
    double inf = int_cr.center - int_cr.radius;
    fesetround(FE_TONEAREST);
    INF_SUP int_is = (INF_SUP) {inf, sup};
    return int_is;
}
...
{
    fesetround(FE_UPWARD);
    double center = (int_is.inf + (int_is.sup - int_is.inf) * 0.5);
    double radius = (center - int_is.inf);
    fesetround(FE_TONEAREST);
    C_R int_cr = (C_R){center, radius};
    return int_cr;
}
